...
class MakeDataSet:
    ...
    def save_meta(self,meta_list):
        ...
        #self.meta.concat(tmp,ignore_index=True)
        #pd.concat(self.meta, tmp, ignore_index=True)
    ...
    def prepare_dataset(self):
        ...
        new_clean_img_path = "C:/Users/farah/Desktop/Lung Cancer Project/LIDC-IDRI-Preprocessing/data/Clean/Image"
        #new_clean_img_path =os.chdir('C://Users//farah//Desktop//Lung Cancer Project//LIDC-IDRI-Preprocessing//data//Clean//Image')
        # Modify path here
        self.clean_path_img = new_clean_img_path
        CLEAN_DIR_IMAGE = Path(self.clean_path_img)
        new_clean_mask_path = "C:/Users/farah/Desktop/Lung Cancer Project/LIDC-IDRI-Preprocessing/data/Clean/Mask"
        #new_clean_mask_path=os.chdir('C://Users//farah//Desktop//Lung Cancer Project//LIDC-IDRI-Preprocessing//data//Clean//Mask')
        # Modify path here
        self.clean_path_mask = new_clean_mask_path
        CLEAN_DIR_MASK = Path(self.clean_path_mask)


        for patient in tqdm(self.IDRI_list):
            pid = patient #LIDC-IDRI-0001~
            patient_image_dir = os.path.join(IMAGE_DIR, pid)
            patient_mask_dir = os.path.join(MASK_DIR, pid)
            patient_clean_dir_image = os.path.join(CLEAN_DIR_IMAGE, pid)
            patient_clean_dir_mask = os.path.join(CLEAN_DIR_MASK, pid)
            ...
            if len(nodules_annotation) > 0:
                ...
                # This current for loop iterates over total number of nodules in a single patient
                    ...
                    for nodule_slice in range(mask.shape[2]):
                        ...
                        #print(nodule_slice)
                        #print(lung_np_array[:,:,nodule_slice].ndim)
                        lung_segmented_np_array = segment_lung(lung_np_array[:,:,nodule_slice])
                        #slice_array = lung_np_array[:,:,nodule_slice]
                        #lung_segmented_np_array = segment_lung(slice_array)
                        ...
                        #np.save(os.path.join(patient_image_dir, nodule_name), lung_segmented_np_array)
                        #np.save(os.path.join(patient_mask_dir, mask_name), mask[:, :, nodule_slice])
                ...
                for slice in range(vol.shape[2]):
                    ...
                    #patient_clean_dir_image = os.path.join('C:/Users/farah/Desktop/Lung Cancer Project/LIDC-IDRI-Preprocessing/data/Clean/Image',f'image_{idx}.npy')
                    #np.save(patient_clean_dir_image,lung_segmented_np_array)
                    os.makedirs(patient_clean_dir_image, exist_ok=True)
                    os.makedirs(patient_clean_dir_mask, exist_ok=True)
                    #np.save(patient_clean_dir_image + '/' + nodule_name, lung_segmented_np_array)
                    #patient_clean_dir_mask = os.path.join('C:/Users/farah/Desktop/Lung Cancer Project/LIDC-IDRI-Preprocessing/data/Clean/Mask',f'image_{idx}.npy')
                    #np.save(patient_clean_dir_mask,lung_mask)
                    ...
                    np.save(os.path.join(patient_clean_dir_image, nodule_name), lung_segmented_np_array)
                    np.save(os.path.join(patient_clean_dir_mask, mask_name), lung_mask)
       

                    #np.save(f"{str(patient_clean_dir_image)}/{nodule_name}.npy", lung_segmented_np_array)
                    #np.save(f"{str(patient_clean_dir_mask)}/{mask_name}.npy", lung_mask)
                    #np.save(patient_clean_dir_image / nodule_name,lung_segmented_np_array)
                    #np.save(patient_clean_dir_mask / mask_name,lung_mask)
